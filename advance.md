...
## Redis持久化
### RDB
RDB，也叫做Redis数据快照，把内存中的所有数据记录到磁盘中
#### fork原理
bgsave开始时会fork主进程得到子进程，子进程共享主进程的内存数据，完成fork后读取内存数据并写入RDB文件

fork采用copy_on_write技术:
- 当主进程执行读操作时，访问共享内存
...
### AOF
AOP全称为追加文件,Redis处理的每一个写命令都会记录在AOF文件
...
appendfsync no
```
通过执行bgrewriteaof命令，可以让AOF文件执行重写功能,以减少AOF文件大小

### RDB和AOF各有自己的优缺点，如果对数据安全性要求较高，在实际开发中往往会结合两者来使用。

| | RDB | AOF |
| :--- | :--- | :--- |
| 持久化方式 | 定时对整体内存做快照 | 记录每一次执行的命令 |
...
- 主观下线: 如果sentinel节点发现某实力未在规定时间响应，则认为该实例 **主观下线**
- 客观下线：若超过指定数量的sentinel都认为该实例下线,则该实例**客观下线**
